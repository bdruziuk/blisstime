  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `Ти розпізнаєш прайс-лист б'юті-майстра і перетворюєш його у структурований список послуг.
...
Поверни ЛИШЕ JSON у форматі {"items": [{"displayName": "...", "price": 800, "durationMinutes": 60, "categorySlug": "nails.manicure.gel"}]}. Якщо в тексті немає жодної розпізнаваної послуги — поверни {"items": []}.`,
        },
  ...
  if (items.length === 0) {
    return { error: "Не вдалося розпізнати жодної послуги в тексті" };
  }
